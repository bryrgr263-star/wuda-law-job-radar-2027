...
- 全国范围筛选：不限制岗位名称或岗位方向，只要 2027 届具体岗位的专业要求包含法学或法律专业即可进入候选。
- 专业限制判断：区分“明确接受法律硕士（非法学）”“可能接受”“要求本科法学”“要求本硕均法学”和“专业限制待核验”。
- 建筑硬排除：建筑施工、工程建设、施工企业、房地产开发建设及相关岗位不会入库。
- 自动巡检：GitHub Actions 每 6 小时集中读取招聘源并批量发布，识别 2027 届且专业要求包含法学或法律的岗位，并按岗位 ID 去重更新；截止日期已过的岗位会自动关闭。
...
`.github/workflows/sync-jobs.yml` 每 6 小时执行一次集中巡检，并在 30 分钟后执行一次幂等补偿巡检；岗位 ID 已存在时不会重复新增。代码更新时也会自动执行一次，也可以在 Actions 页面手动触发。
...
- 只收录招聘年度为 2027、专业要求包含法学或法律专业的具体岗位，不预先限制岗位名称或方向。
- `job_id` 已存在时仅更新岗位信息，不新增重复记录。
- 高置信度岗位直接公开；来源或专业要求不清时带标记公开。
- 招聘页面消失、明确截止或截止日期已过时，更新为“已截止”或“已关闭”。
- 聚合网站只用于发现岗位；能找到官方投递页时，投递按钮必须指向官方页面。
- 国资委、人社部央企平台和烟草总入口若拒绝海外云端访问，改由国聘、应届生求职网、智联及各招聘单位官网交叉发现；中科院软件所使用其 HTTP 官方备用入口。
