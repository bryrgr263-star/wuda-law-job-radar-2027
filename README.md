...
- 单求职者账号：只有授权邮箱可以登录并管理收藏、投递状态、投递时间和私人备注。
...
- 自动巡检：GitHub Actions 每 6 小时读取官方招聘源，识别 2027 届岗位并按岗位 ID 去重更新。
...
### 4. Supabase Cron 备用方案

修改 `supabase/schedule.sql` 中的站点地址和 `CRON_SECRET`，随后在 Supabase SQL Editor 执行。任务将在北京时间每天 00:00、06:00、12:00、18:00 对应的 UTC 时刻附近运行；如果希望严格按北京时间执行，可将 Cron 表达式调整为 UTC 对应时间。

Vercel 免费计划中的 `vercel.json` 仅保留每天一次的兜底巡检。主要的六小时任务由 GitHub Actions 执行；`supabase/schedule.sql` 只作为备用方案，不必同时启用。
